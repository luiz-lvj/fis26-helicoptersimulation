...
class Cone(object):
    def __init__(self, densidade, raio, geratriz, v_inicial, altura_inicial, vertice, v_ang, a_ang, iteracoes):
        ...
        self.p_direita = np.array(
            [self.raio, -self.geratriz * cos(self.angulo)])
        self.p_esquerda = np.array(
            [-self.raio, -self.geratriz * cos(self.angulo)])
    ...
    def draw(self, screen):
        poligono_pontos = [self.vertice, self.vertice +
                           self.p_direita, self.vertice + self.p_esquerda]
        pygame.draw.polygon(screen, (0, 0, 255), poligono_pontos)

    def move(self):
        dphi_dot = (self.a_ang * self.drotacao +
                    self.v_ang**2 * sin(self.rotacao))
        self.rotacao = self.rotacao + self.dtempo * self.drotacao
        self.drotacao = self.drotacao - self.dtempo * dphi_dot

        forca = 0.5 * 1.293 * pi * (self.raio**2) * \
            (self.velocidade**2) / 2 * cos(self.rotacao)
        ...
        self.velocidade = self.velocidade - \
            (self.gravidade - forca / self.massa) * self.dtempo
